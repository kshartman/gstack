...
_UPGRADE_MODE=$(${ctx.paths.binDir}/gstack-config get upgrade_mode 2>/dev/null || true)
_DEV_REPO=$(${ctx.paths.binDir}/gstack-config get dev_repo_path 2>/dev/null || true)
_DEV_VER=""
...
- Otherwise: read \`${ctx.paths.skillRoot}/gstack-upgrade/SKILL.md\` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).
